...
base_binary$G2[base$G2 < 10] = 0
base_binary$G2[base$G2 >= 10] = 1
base_binary$G1[base$G1 < 10] = 0
base_binary$G1[base$G1 >= 10] = 1
...
hist(base_binary$G3, col = "blue", breaks = 2) #ver distribuicao

barplot(prop.table(table(base_binary$G3)))
...
M5=fit(G3~.,df_train,model="randomForest", task="class",search = s)
P5=predict(M5,df_test)
